...
export async function smokeChromiumSession(options) {
  ...
  try {
    const result = await requestJson(`${serverUrl.href}status`, {}, 15000, options.signal);
  ...
  if (sessionId) {
    try {
      const result = await requestJson(
        ...
        { method: "DELETE" },
        options.sessionCleanupTimeoutMs ?? 30000,
      );
      deleteSession = {
        ok: result.ok,
        status: result.status,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const report = await smokeChromiumSession(parseArguments(outputOptions.args));
